...
def test_delete_multiple_columns(client):
    initial_columns = upload_file_helper(client)
    assert len(initial_columns) >= 2, "Sample CSV must have at least 2 columns for this test"
...
def test_delete_column_no_file(client):
    ...
    assert response.status_code == 400 # Expecting failure
    json_data = response.get_json()
    assert 'No file uploaded' in json_data['error'] or 'File not found' in json_data['error'] # Accept either error message
...
def test_upload_no_selected_file(client):
    ...
    data = {
        'file': (open('tests/sample.csv', 'rb'), '') # Empty filename
    }
    response = client.post('/upload', data=data, content_type='multipart/form-data')
    assert response.status_code == 400
    json_data = response.get_json()
...
def test_upload_malformed_csv(client):
    data = {
        'file': (open('tests/malformed.csv', 'rb'), 'malformed.csv')
    }
    response = client.post('/upload', data=data, content_type='multipart/form-data')
    assert response.status_code == 400 # Expecting error during CSV parsing
    json_data = response.get_json()
    assert 'error' in json_data
    assert 'Error reading CSV' in json_data['error'] # Check for the specific pandas error message
...
def test_show_classification_success(client):
    ...
    assert len(json_data['classifications']) > 0 # Make sure default classifications are sent
...
def test_show_classification_no_file(client):
    ...
    assert response.status_code == 400
    json_data = response.get_json()
    assert 'No file uploaded' in json_data['error'] or 'File not found' in json_data['error']
...
def test_check_empty_fields_name(client):
    ...
    response = client.post('/check-empty-fields', json={'columns': ['Name'], 'classificationType': 'Non-categorical'}) # Assuming Name is Non-categorical default
    assert response.status_code == 200
    json_data = response.get_json()
    assert json_data['success'] is True
...
def test_check_empty_fields_categorical(client):
    ...
    response = client.post('/check-empty-fields', json={'columns': ['Category'], 'classificationType': 'Categorical'})
    assert response.status_code == 200
    json_data = response.get_json()
    assert json_data['success'] is True
    assert json_data['hasEmptyFields'] is True
...
def test_check_empty_fields_numerical(client):
    upload_file_helper(client, filename='sample_with_empties.csv')
    response = client.post('/check-empty-fields', json={'columns': ['Value'], 'classificationType': 'Numerical'})
    assert response.status_code == 200
    json_data = response.get_json()
    assert json_data['success'] is True
    assert json_data['hasEmptyFields'] is True
...
def test_check_empty_fields_date(client):
    upload_file_helper(client, filename='sample_with_empties.csv')
    response = client.post('/check-empty-fields', json={'columns': ['Date'], 'classificationType': 'Date'})
    assert response.status_code == 200
    json_data = response.get_json()
    assert json_data['success'] is True
    assert json_data['hasEmptyFields'] is True
...
def test_check_empty_fields_no_empties(client):
    ...
    response = client.post('/check-empty-fields', json={'columns': ['Name'], 'classificationType': 'Non-categorical', 'rows': [0]}) # Check only first row (Alice)
    ...
    response = client.post('/check-empty-fields', json={'columns': ['Name'], 'classificationType': 'Non-categorical'})
    json_data = response.get_json()
...
def test_handle_empty_name_fields_delete_rows(client):
    upload_file_helper(client, filename='sample_with_empties.csv')
    response = client.post('/handle-empty-name-fields', json={'nameEmptyHandling': {'Name': 'delete-empty-rows'}})
    assert response.status_code == 200
    json_data = response.get_json()
    assert json_data['success'] is True
    assert 'table' in json_data
    # Check that the row with empty Name is gone (original index 4)
    assert '><td></td>' not in json_data['table'] # Crude check, better would be parsing HTML or checking dataframe state

def test_handle_empty_name_fields_fill_none(client):
    upload_file_helper(client, filename='sample_with_empties.csv')
    response = client.post('/handle-empty-name-fields', json={'nameEmptyHandling': {'Name': 'fill-with-"none"'}})
    assert response.status_code == 200
    json_data = response.get_json()
    assert json_data['success'] is True
...
def test_handle_empty_fields_no_file(client):
    response = client.post('/check-empty-fields', json={'columns': ['Any'], 'classificationType': 'Categorical'})
    assert response.status_code == 400
    json_data = response.get_json()
